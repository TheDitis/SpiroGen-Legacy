...
#         if isinstance(colors, (ColorScheme, ColorSchemeDict)):
#             colors = colors.hex
...
class FlowerPattern(PolarPattern):
    def __init__(self, npetals, innerdepth=3, size=50, poly=500, color='green',
        ...
        if isinstance(color, (ColorScheme, ColorSchemeDict)):
            color = color.hex
...
class DrawPath:
    ...
    def colorset(self):
        if isinstance(self.colors, (ColorScheme, ColorSchemeDict)):
...
rainbow1 = ColorSchemeDict(rainbow, 100)
newcol1 = ColorSchemeDict(try2, 120, symetrical=True)
...
cloud1 = Transform(path1).generatepointcloud(2, 3)
DrawPath(cloud1, 1, 'white', lines=False, dots=True)
DrawPath(path1, 2, newcol1, colordist=50, lines=True) #, dots=True)
