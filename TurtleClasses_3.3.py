...
class LVL2:
    ...
    def sin_avg_point_rotation(strands=20, xshift=10, yshift=0,
        ...
        if showpoint is True and rotation_point is not None:
            drawpoint(rotation_point, 'white')
            drawpoint(center, 'gray')
        if getpoint is True:
...
# Analyze(ra2).crosspoint(show=True)
...
# LVL2.layered_flowers(50, rotate=1, color=rainbow1)
# LVL2.antenas()

# cloud1 = Transform(waves).generatepointcloud(10, 10, exp=6)
...
# spi = LVL2.sin_avg_point_rotation(60, 0, 0, 2, colors=rainbow1)

# waves = LVL2.sin_avg_point_rotation(32, 5, 0, rotate=2, rotaterate=1,ampshift=5, colors=rainbow1, length=30, cosine=True)
flower = LVL2.layered_flowers(layers=80, npetals=6, innerdepth=3, sizefactor=2, pensize=2, rotate=1, colors=rainbow1)
